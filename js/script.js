...
if (heure > 19) {
  ...
  if (window.matchMedia('(max-width: 599px)').matches) {
    document.getElementById('animMobile').style.background =
      "url('../images/svg/lune-mobile.svg') no-repeat right top";
  } else {
    let x = 0;
    let y = 0;
    ...
    posY = 0;
    anim = true;
    oldpos = '../images/svg/lune.svg';
    function Bouge_Image() {
      if (document.getElementById && anim) {
        posX = posX + (x - posX + 20) / 15;
        posY = posY + (y - posY + 20) / 15;
        if (posX < x) newpos = '../images/svg/lune.svg';
        else newpos = '../images/svg/lune.svg';
        if (newpos != oldpos) {
    ...
    if (document.getElementById) {
      ...
      document.write(
        '<img class="imagenoel" style="width:80%;" src="../images/svg/lune.svg" style="border:0px" name="tete" alt="Cliquez ici pour faire disparaître" />'
      );
      document.write('</a>');
      document.write('</div>');
    }
